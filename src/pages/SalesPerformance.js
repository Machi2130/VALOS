...
export default function SalesPerformance() {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.log("✅ Stats loaded from /stats endpoint:", statsRes.data);
        
        // Handle the actual API response format with by_status object
        ...
        setStats({
          ...
          converted: byStatus.won || byStatus.converted || 0,
          lost: byStatus.lost || 0,
        });
      } catch (statsErr) {
        // If stats endpoint doesn't exist, fetch all leads
        console.log("⚠️ Stats endpoint not available, fetching all leads...");
        ...
        const leads = leadsRes.data.items || leadsRes.data || [];
        console.log("✅ Loaded", leads.length, "leads for counting");
  ...
  useEffect(() => {
    console.log("📊 SalesPerformance component mounted - setting up listeners");
    ...
    // 🔔 Listen for lead updates - using a persistent handler
    const handleLeadsUpdated = (e) => {
      console.log("🔔 RECEIVED leadsUpdated event - refreshing sales performance...");
    ...
    window.addEventListener('leadsUpdated', handleLeadsUpdated, true);
    console.log("✅ Event listener registered for 'leadsUpdated'");

    return () => {
      console.log("🧹 SalesPerformance component unmounting - cleaning up");
      clearInterval(interval);
      window.removeEventListener('leadsUpdated', handleLeadsUpdated, true);
    };
  }, [loadStats]);
